...
class DQN(QN):
    ...
    def build(self):
        ...
        #self.q, self.small_saver1, self.load_file, self.save_file, self.saver_for_transfer = self.get_q_values_op(s, scope="q", reuse=False)
        self.q = self.get_q_values_op(s, scope="q", reuse=False)

        # compute Q values of next state
        sp = self.process_state(self.sp)
        #self.target_q, _, _, _, _ = self.get_q_values_op(sp, scope="target_q", reuse=False)
    ...
    # If transfer learning, need to specify saver and load_file to load smaller model
    def initialize(self):
        """
        ...
        self.sess.run(init)
        # ADDED THE BELOW TWO LINES TO WORK WITH SAVE / LOADING
        #if not self.small_saver1 is None:
        #  small_saver1.restore(sess, self.load_file)
    ...
    def save(self):
        ...
        ##self.saver_for_transfer.save(self.sess, self.save_file)
